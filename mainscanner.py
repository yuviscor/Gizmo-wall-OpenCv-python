...
while True:
    ...
    thres = utils1.valTrackbars()  # GET TRACK BAR VALUES FOR THRESHOLDS
    ...
    biggest, maxArea = utils1.biggestContour(
        contours)  # FIND THE BIGGEST CONTOUR
    if biggest.size != 0:
        biggest = utils1.reorder(biggest)
        # DRAW THE BIGGEST CONTOUR
        cv2.drawContours(imgBigContour, biggest, -1, (0, 255, 0), 20)
        imgBigContour = utils1.drawRectangle(imgBigContour, biggest, 2)
    ...
    stackedImage = utils1.stackImages(imageArray, 0.75, lables)
    cv2.imshow("Result", stackedImage)

    # SAVE IMAGE WHEN 's' key is pressed
    if cv2.waitKey(1) & 0xFF == ord('s'):
        cv2.imwrite("image/Yuvraj"+str(count)+".jpg", imgWarpColored)
